...
function DigiSize(number) {
  ...
  if (number < 99) {
    ...
    while (i <= 100) {
      ...
      digiy = digi2 * 10;
      console.log(digi2);
      let digi3 = number - digiy - digix;
      console.log(digi3);

      digiSum = digi1 * digi2;
      console.log(digiSum);
      digiSum = digiSum * digi3;
      console.log(digiSum);
      number = digiSum;
      console.log(digiSum);
      b = b + 1;
...
function evenNumbersA(array, number) {
  // good luck
...
function repeatStr(n, s) {
  const out = s.repeat(n);
  return out;
}

console.log(repeatStr(5, "Arne"));
...
function descendingOrder(n) {
  ...
  for (let i = 0; i < narr.length; i++) {
    narrX.push(narr[i]); //array
  }

  let narrY = narrX.sort((a, b) => b - a); // array sortiert
  const nbb = narrY.join("");
  let naxx = Number(nbb);
  return naxx;
}
console.log(descendingOrder(112221));
